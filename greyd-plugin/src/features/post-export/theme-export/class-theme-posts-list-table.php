...
class Theme_Posts_List_Table extends \WP_List_Table {
	...
	public function get_columns() {
		return apply_filters( 'greyd_theme_manage_assets_columns', array(
	...
	protected function get_views() {
		...
		if (
			...
			$views['current_theme'] = sprintf(
				...
				esc_html__( 'Current Theme', 'greyd_hub' ),
				$current_theme_count
			);
			$views['inactive'] = sprintf(
				...
				esc_html__( 'Not Active', 'greyd_hub' ),
	...
	public function column_title( $post ) {
		...
		$edit_post_link  = Helper::get_edit_post_link( $post );
		...
		if ( $is_trash ) {
			...
			$row_actions['delete'] = sprintf(
				...
				$delete_link,
				esc_attr( sprintf( __( 'Delete &#8220;%s&#8221; permanently', 'greyd_hub' ), $post->post_title ) ),
				__( 'Delete Permanently', 'greyd_hub' )
			);
			// Export
			$row_actions['greyd_export'] = sprintf(
		...
		$template_theme = Helper::get_wp_template_theme( $post );
		if ( $template_theme ) {
			...
			if ( get_option( 'stylesheet' ) != $template_theme ) {
				...
				if ( $post->post_type == 'wp_template' || $post->post_type == 'wp_template_part' ) {
					$row_actions['switch_template_theme'] = sprintf(
						...
						$post->ID,
						__( 'Assign to current theme', 'greyd_hub' )
					);
				}
				else if ( $post->post_type == 'wp_global_styles' ) {
					$row_actions['switch_global_styles'] = sprintf(
						'<a style="cursor:pointer;" onclick="greyd.postExport.openSwitchGlobalStyles(this, \''.$template_theme.'\');" data-post_id="%s">%s</a>',
						$post->ID,
						__( 'Assign to current theme', 'greyd_hub' )
					);
				}
	...
	public function column_default( $item, $column_name ) {
		...
		return apply_filters( 'greyd_theme_manage_assets_column_default', $column_name, $post_id );
	}
	...
	public function get_bulk_actions() {
		...
		if ( $is_trash ) {
			$actions = array(
				'untrash' => __( 'Restore', 'greyd_hub' ),
				'delete'  => __( 'Delete Permanently', 'greyd_hub' ),
			);
		} else {
			$actions = array(
				'trash'        => __( 'Move to the trash', 'greyd_hub' ),
				'export'       => __( 'Export', 'greyd_hub' ),
				'switch_theme' => __( 'Assign to current theme', 'greyd_hub' ),
			);
		}
		return $actions;
	}
	...
	public function process_bulk_action() {
		...
		if ( isset( $_GET['action'] ) && isset( $_GET['post'] ) && isset( $_GET['post_status'] ) && $_GET['post_status'] === 'trash' ) {
			...
			if ( $single_action === 'delete' && current_user_can( 'delete_post', $post_id ) ) {
				...
				if ( $post ) {
					...
					if ( $result ) {
						Helper::show_message( 
							sprintf( __( 'The post "%s" has been permanently deleted.', 'greyd_hub' ), $post->post_title ), 
							'success' 
						);
					} else {
						Helper::show_message( __( 'Error occurred when deleting the post permanently.', 'greyd_hub' ), 'error' );
					}
				}
				return;
			}
		...
		$notices = array(
			...
			'untrash'          => array(
				'success' => __( 'The posts %s have been successfully restored.', 'greyd_hub' ),
				'fail'    => __( 'Errors occurred when restoring the posts.', 'greyd_hub' ),
			),
			'delete'           => array(
				'success' => __( 'The posts %s have been permanently deleted.', 'greyd_hub' ),
				'fail'    => __( 'Errors occurred when deleting the posts permanently.', 'greyd_hub' ),
			),
			'switch_theme'     => array(
				'success' => __( 'The posts %s have been successfully assigned to the current theme', 'greyd_hub' ),
				'fail'    => __( 'Errors occurred when assigning posts to the current theme.', 'greyd_hub' ),
			),
			'export'           => array(
