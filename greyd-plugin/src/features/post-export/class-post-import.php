...
class Post_Import {
	...
	/**
	 * Holds all Preparred_Post objects for import.
	 *
	 * @var Preparred_Post[]
	 */
	 ...
	 * Import posts
	 *
	 * @param Preparred_Post[] $posts       Preparred post objects.
	...
	public static function import_posts( $posts, $conflict_actions = array(), $zip_file = '' ) {
		...
		foreach ( $posts as $post_id => $post ) {
			...
			if (
				...
				if ( $language_switched ) {
					do_action( 'greyd_post_export_log', '  - language supported and switched!' );
					...
					if ( ! empty( $translation_post_ids ) ) {
						foreach ( (array) $translation_post_ids as $lang => $translated_post_id ) {
							if ( isset( self::$posts[ $translated_post_id ] ) ) {
								...
								self::$posts[ $post_id ] = self::$posts[ $translated_post_id ];
								$skip_this_translation   = true;
								break;
							}
						}
			 ...
			 */
			$import_action = apply_filters('greyd_import_action', isset( $post->import_action ) ? $post->import_action : 'insert', $post, $existing_post_id );
			...
			if ( $conflict_action === 'replace' ) {
				...
				if ( $existing_post_id ) {
					...
					do_action( 'greyd_post_export_log', sprintf( '  - replace existing post with ID: %s.', $existing_post_id ) );
				...
				if ( $existing_post_id ) {
					...
					if ( $postarr['post_type'] === 'attachment' ) {
						...
						if ( ! file_exists( $file_path ) ) {
							...
							do_action( 'greyd_post_export_log', '  - file does not exist, we import the attachment even if the post should be skipped.' );
						}
						else {
							if ( isset( $post->media ) ) {
								...
								if ( isset( $post->media['path'] ) ) {
									self::add_attachment_files_to_replace_strings( $post->media['path'], $file_path );
								}
							}
						}
					}
					...
					if ( $skip ) {
						...
						do_action( 'greyd_post_export_log', sprintf( '  - skip this post and use the existing post: %s.', $existing_post_id ) );
			...
			elseif ( $result ) {
				 ...
				 * post correctly and in order for actions and filters are working as
				 * expected, like setting taxonomy terms.
				 */
				if ( isset( $post_language_code ) ) {
					Post_Export_Helper::set_post_lang( $result, $post_language_code );
				}
			}
		}
		 ...
		 * 
		 * We need to always register the post types and taxonomies, in order
		 * for all terms to be found during term dependant functions like
		 * 'replace_nested_terms()' and 'set_taxonomy_terms()'.
		 */
	 ...
	 * @param Preparred_Post $post  Preparred post object.
	...
	public static function insert_attachment( $postarr, $media_file_info, $zip_file='' ) {
		...
		do_action( 'greyd_post_export_log', '  - regenerate attachment meta data.' );
		...
		self::add_attachment_files_to_replace_strings( $media_file_info['path'], $file );

		
		// do_action( 'greyd_post_export_log', '  - attachment meta data updated: ', $attach_data );
	 ...
	 * 
	 * @param string $old_file   The old file path.
	 * @param string $new_file   The new file path.
	 */
	public static function add_attachment_files_to_replace_strings( $old_file, $new_file ) {

		if (
			strpos( $old_file, 'wp-content/' ) !== false
			&& strpos( $new_file, 'wp-content/' ) !== false
		) {
			// compare the file paths after 'wp-content' to check if the files are the same
			$new_file_path = explode( 'wp-content/', $new_file )[1];
			$old_file_path = explode( 'wp-content/', $old_file )[1];
		} else {
		...
		$old_file_path = preg_replace( '/\.[^.]+$/', '', $old_file_path );
	...
	public static function replace_nested_posts( $subject, $post ) {
		...
		foreach ( $nested as $nested_id => $nested_postarr ) {
			...
			do_action( 'greyd_post_export_log', sprintf( "  - replace '%s' with '%s'.", $marked_string, $replace_string ) );
		}
	...
	public static function replace_nested_terms( $subject, $post ) {
		...
		foreach ( $nested as $nested_id => $nested_term_object ) {
			...
			if ( empty( $nested_term_object ) ) {
				do_action( 'greyd_post_export_log', "  - term object is not set correctly.", $nested_term_object );
				continue;
			}
			...
			if ( $term_object ) {
				...
				if ( ! empty( $new_term_ids ) ) {
					...
					do_action( 'greyd_post_export_log', "  - term of taxonomy '{$nested_term_object->taxonomy}' with slug '{$nested_term_object->slug}' inserted.", $new_term_ids );
				} else {
					$replace_string = $nested_id;
		...
		if ( !empty( self::$collected_replace_strings ) ) {
			foreach ( self::$collected_replace_strings as $original => $new ) {
				if ( strpos( $subject, $original ) !== false ) {
					...
					if ( $log ) {
						do_action( 'greyd_post_export_log', sprintf( "  - '%s' was replaced with '%s'.", $original, $new ) );
					}
				}
				else {
					if ( $log ) {
						do_action( 'greyd_post_export_log', sprintf( "  - '%s' was not found.", $original ) );
					}
				}
			}
		}
	 ...
	 * @param Preparred_Post $post  The preparred post object (optional).
	 */
	...
	public static function insert_taxonomy_terms( string $taxonomy, $terms, $post_type=null ) {
		...
		// skip polylang 'post_translations' - will be set new in set_translations()
		if ( $taxonomy == 'post_translations' ) return false;
		...
		if ( ! taxonomy_exists( $taxonomy ) ) {
			...
			$taxonomies = Post_Export_Helper::get_dynamic_taxonomies( $post_type );

			// register taxonomy if it is dynamic
			if ( in_array( $taxonomy, $taxonomies ) && ! taxonomy_exists( $taxonomy ) ) {
				$result = register_taxonomy( $taxonomy, $post_type ? $post_type : 'post' );
				if ( is_wp_error( $result ) ) {
			...
			// return if taxonomy still doesn't exist
			if ( ! in_array( $taxonomy, $taxonomies ) && ! taxonomy_exists( $taxonomy ) ) {
				return false;
			}
		}
	...
	public static function set_taxonomy_term_parents( $taxonomy, $term_array, $prepared ) {
		...
		if ( empty( $term ) ) {
			...
			if ( is_wp_error( $term ) ) {
				do_action( 'greyd_post_export_log', "      - parent term '{$term_array['name']}' of taxonomy '$taxonomy' could not be inserted: " . $term->get_error_message() );
			}
			else {
				$term_id = $term['term_id'];
				do_action( 'greyd_post_export_log', "      - parent term '{$term_array['name']}' of taxonomy '$taxonomy' inserted with id '{$term['term_id']}'." );
			}
		}
		else {
			$term_id = $term->term_id;
			do_action( 'greyd_post_export_log', "      - parent term '{$term_array['name']}' of taxonomy '$taxonomy' found with id {$term->term_id}." );
		}
	 ...
	 * @param Preparred_Post $post  Old Preparred_Post object (Post ID might differ).
	 *
	 * @return bool
	 */
	public static function set_translations( $post_id, $post ) {
		...
		// do not allow polylang -> wpml
		if ( $tool == 'wpml' &&  strval( $language['tool'] ) != $tool ) {
			do_action( 'greyd_post_export_log', "\r\n" . 'polylang to wpml conversion is not supported.' );
			return false;
		}
		...
		switch ( $tool ) {
			...
			case 'polylang':
				...
				$post_ids[$code] = $post->ID;
				// check if all translations are imported
				$check = 0;
				foreach ( $post_ids as $lang => $id ) {
					// get id on this stage
					$current_id = isset( self::$posts[$id] ) ? self::$posts[$id] : null;
					if ( !empty($current_id) ) {
						$post_ids[$lang] = $current_id;
						$check++;
					}
				}
				if ( count($post_ids) != $check ) {
					return false;
				}
				// make sure the language term is set.
				$term = term_exists( $code, 'language' );
				if ( $term ) {
					wp_set_post_terms( $post_id, array( (int)$term['term_id'] ), 'language', false );
				}
				// set the translations
				pll_save_post_translations( $post_ids );
				do_action( 'greyd_post_export_log', "\r\n" . 'Set translations:', $post_ids );
	 ...
	 *
	 * @param Preparred_Post[] $posts  Posts keyed by ID
	 *
