 ...
 * @since 1.7.0
 */
function greyd_register_anchor_block() {
	...
	if ( function_exists( 'wp_register_script' ) ) {
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'greyd-anchor-editor-script', 'greyd_hub', trailingslashit( WP_PLUGIN_DIR ) . 'greyd-plugin/languages' );
		}
	}
	...
	if ( function_exists( 'register_block_type' ) ) {
		register_block_type(
			'greyd/anchor',
			array(
				'editor_script' => 'greyd-anchor-editor-script',
				'style'         => 'greyd-anchor-frontend-style'
			)
		);
	}
}
add_action( 'init', 'greyd_register_anchor_block', 99 );
